...
These are auto-created by the database migration on first boot — no setup wizard, no separate admin-bootstrap step. **Change the password immediately** via Settings → Users → Edit `admin`. Add additional users from the same page.
